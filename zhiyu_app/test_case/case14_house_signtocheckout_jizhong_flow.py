...
class SignToCheckout(unittest.TestCase):
    ...
    def test_sign_checkout(self):
        ...
        self.orders.order_confirm_flow(data.community_jizhong_ui,data.room_no_1001, data.remark, money_1=21860.00)
        """
        # 转到web端财务确认
        # self.driver_web = driver.Driver().driver_web_session(url_zhiyu_test)
        # self.login_web = login_w.PageActionLogin(self.driver_web)
        ...
        """
        # app端财务确认
        self.orders.caiwu_pay_confirm(data.community_jizhong_ui, data.room_no_1001, money_t=21860.00)
        # 转到app执行退房
        self.home.click_tab_house_status()
        ...
        self.checkout.checkout_flow_jizhong(data.item_bed, data.reason_bed, data.room_status)
